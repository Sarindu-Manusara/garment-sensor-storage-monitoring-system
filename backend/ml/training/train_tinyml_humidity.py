...
def resolve_sensor_csv_path(csv_path: Path | None) -> Path | None:
    if csv_path:
        return csv_path
    ...
    for candidate_name in candidate_names:
        candidate_path = PROJECT_ROOT / candidate_name
        if candidate_path.exists():
            return candidate_path
    return None


def infer_resample_rule(frame: pd.DataFrame) -> str:
    timestamp_series = pd.to_datetime(frame.get("timestamp"), utc=True, errors="coerce").dropna().sort_values()
    if len(timestamp_series) < 2:
        return DEFAULT_TINYML_RESAMPLE_RULE

    diffs = timestamp_series.diff().dropna().dt.total_seconds()
    if diffs.empty:
        return DEFAULT_TINYML_RESAMPLE_RULE
    ...
    if median_seconds <= 10:
        rounded_seconds = max(1, int(round(median_seconds / 5.0) * 5))
        return f"{rounded_seconds}s"
    if median_seconds < 60:
        return f"{max(1, int(round(median_seconds)))}s"
    if median_seconds < 3600:
        rounded_minutes = max(1, int(round(median_seconds / 60.0)))
        return f"{rounded_minutes}min"
    rounded_hours = max(1, int(round(median_seconds / 3600.0)))
    return f"{rounded_hours}h"


def load_sensor_frame(args) -> pd.DataFrame:
    csv_path = resolve_sensor_csv_path(args.csv)
    if csv_path:
        print(f"Loading TinyML training data from CSV: {csv_path}")
    ...
    if "zone" in frame.columns and args.zone:
        zone_frame = frame.loc[frame["zone"] == args.zone].copy()
    ...
    rule = args.resample or infer_resample_rule(frame)
    print(f"Resampling TinyML training data with rule: {rule}")
    frame = frame.set_index("timestamp").sort_index()
    numeric_columns = ["temperature", "humidity", "lightLux", "dustMgPerM3", "mq135AirQualityDeviation", "mq135Raw"]
    frame[numeric_columns] = frame[numeric_columns].apply(pd.to_numeric, errors="coerce")
    frame = frame.resample(rule).mean(numeric_only=True)
    frame[numeric_columns] = frame[numeric_columns].interpolate(limit=2, limit_direction="both")
    frame = frame.dropna(subset=["temperature", "humidity", "lightLux", "dustMgPerM3", "mq135AirQualityDeviation"])
...
def normalize_targets(y_train, y_validation, y_test):
    ...
    def transform(values):
        return ((values - target_mean) / target_scale).astype(np.float32)

    return transform(y_train), transform(y_validation), transform(y_test), target_mean, target_scale
...
def train_candidate(tf, model, X_train, y_train, X_validation, y_validation, target_mean: float, target_scale: float):
    ...
    validation_predictions = (model.predict(X_validation, verbose=0).reshape(-1) * target_scale) + target_mean
    validation_targets = (y_validation * target_scale) + target_mean
...
def evaluate_quantized_model(
    ...
    for sample in X_test:
        ...
        predictions.append(float(output.squeeze() * target_scale + target_mean))

    return evaluate_predictions(y_test, np.array(predictions, dtype=np.float32))


def select_deployable_candidate(candidate_reports: Dict[str, Dict[str, object]]) -> Tuple[str, Dict[str, object], str]:
    ...
    compatible_candidates.sort(key=lambda item: float(item[1]["validation_mae"]))
    best_name, best_report = compatible_candidates[0]
    best_validation_mae = float(best_report["validation_mae"])

    mlp_report = candidate_reports.get("mlp")
    if (
        mlp_report
        and bool(mlp_report["compatible"])
        and float(mlp_report["validation_mae"]) <= best_validation_mae * 1.1
    ):
        return "mlp", mlp_report, "Selected MLP because it stays within 10% of the best validation MAE while keeping the most deployment-friendly operator set."

    return best_name, best_report, "Selected the lowest-validation-error compatible candidate because it materially outperformed the more deployment-friendly alternative."
    ...
    output_path.write_text(
        "\n".join(
            [
                ...
                f"static constexpr float kHumidityTargetMean = {target_mean:.8f}f;",
                f"static constexpr float kHumidityTargetScale = {target_scale:.8f}f;",
                "",
            ]
        ),
        encoding="utf-8",
...
def main() -> None:
    ...
    parser.add_argument("--resample", default=None, help="Optional pandas resample rule. If omitted, the trainer infers it from the dataset cadence.")
    ...
    resample_rule = args.resample or infer_resample_rule(frame)
    max_gap_seconds = pd.Timedelta(resample_rule).total_seconds() * 2.5
    ...
    y_train_normalized, y_validation_normalized, y_test_normalized, target_mean, target_scale = normalize_targets(
        y_train,
        y_validation,
        y_test,
    )
    ...
    for name, model in candidates.items():
        ...
        float_test_predictions = (trained["model"].predict(X_test, verbose=0).reshape(-1) * target_scale) + target_mean
        float_test_metrics = evaluate_predictions(y_test, float_test_predictions)
        ...
        quantized_metrics = evaluate_quantized_model(
            tf,
            tflite_path,
            X_test,
            y_test,
    ...
    selected_name, selected_report, selection_reason = select_deployable_candidate(candidate_reports)
